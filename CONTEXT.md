...
- Sanity checks: Nyquist check (warns if `|lo_offset_sw| + chirp_bw/2 >= sample_rate/2`), analog filter check (warns if signal extends outside `RF.bw` passband), pulse length compatibility check between GENERATE and GENERATE1.
...
- Prints a TX summary at startup showing LO frequency, SW LO offset, RF center, RF range/bandwidth, TX gain, and phase dither status for each active channel.
